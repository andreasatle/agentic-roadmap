...
def test_planner_rejects_state_payload():
    planner_input = WriterPlannerInput(
        task=DraftSectionTask(node_id="Intro", section_name="Intro", purpose="intro", requirements=[""]),
    )
...
def test_worker_input_rejects_unexpected_fields():
    with pytest.raises(Exception):
        DraftWorkerInput(task=DraftSectionTask(node_id="Intro", section_name="Intro", purpose="", requirements=[""]), extra_field="nope")  # type: ignore[arg-type]
    assert DraftWorkerInput.model_validate(
        {"task": {"kind": "draft_section", "node_id": "Intro", "section_name": "Intro", "purpose": "", "requirements": [""]}}
    )
    with pytest.raises(Exception):
        RefineWorkerInput(task=RefineSectionTask(node_id="Intro", section_name="Intro", purpose="", requirements=[""]), extra_field="nope")  # type: ignore[arg-type]


def test_worker_does_not_mutate_state():
    worker_input = DraftWorkerInput(task=DraftSectionTask(node_id="Intro", section_name="Intro", purpose="", requirements=[""]))
...
def test_lazy_clients_no_api_key_required(monkeypatch):
    ...
    worker_input = RefineWorkerInput(task=RefineSectionTask(node_id="Intro", section_name="Intro", purpose="", requirements=[""]))
